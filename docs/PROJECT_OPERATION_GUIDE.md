...
- 투자자 수급 외부 수집은 비활성화되어 있으므로 수급 컨텍스트는 빈 상태로 유지한다.
- 설정에 따라 DART 공시와 Naver 뉴스 원문을 수집한다.
...
| 외부 coverage 부족 | 일부 시장·공시·뉴스 데이터 수집 실패 | 네트워크/API 키 확인, 보고서의 coverage 확인 |
| `pykrx` 모듈이 없음 | 정상 상태이며 현재 사용하지 않는 의존성 | 설치하지 않아도 됨. 실데이터 OHLCV는 `yfinance` 사용 |
