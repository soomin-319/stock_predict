...
> 분석 대상 실행: `run_id 20260625T011048Z_7059fbf5` (git `fd772b4`), 5종목(삼성전자·SK하이닉스·NAVER·카카오·LG화학), 다음 거래일 1일 예측.
...
- **AUC 0.508**이 가장 깔끔한 진단: 기저비율을 제거하면 상승/하락을 무작위보다 못 가린다.
...
→ `foreign_net_buy / institution_net_buy` 및 의존 피처 약 15개(`foreign_buy_signal`, `institution_net_buy_z20`, `smart_money_strength`, `*_high_conviction_buy_flag`, `investor_event_score`, `jongbae_score` …)가 전부 상수 0.
한국 시장에서 단기 예측력 있는 신호원 한 묶음이 통째로 죽어 있는 셈.
...
- 이유: 보정 표본의 92%(2,432/2,641)가 0.4~0.5에 몰려 있고 0.9~1.0 구간은 단 1표본. 현재 5종목은 **표본 ≈1개짜리 극단 꼬리**로 들어가 보정이 무의미한 외삽.
...
2. **횡단면 랭킹으로 목표 재정의** — 절대 방향(불가능에 가까움) 대신 "상대적 우위 종목 선택"(rank_ic 0.092의 약한 + 신호를 키움).
3. **지평선 확대** — 1일 → 주간/월간. 드리프트가 노이즈 위로 누적.
4. **정칙화 + 캘리브레이션 수축** — `reg_alpha/lambda` > 0, 희소 꼬리는 0.5 쪽으로 shrinkage(또는 신뢰구간 동반 표기).
5. **레짐 조건부 모델** — 추세/변동성 레짐별 분리 학습.
