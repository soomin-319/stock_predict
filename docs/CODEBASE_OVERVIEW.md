...
- **보존·정리 정책**: `result/runs`·`result/test`·`result/runtime/logs`의 보존 기간·보호
  규칙·용량 한계는 `src/utils/result_cleanup.py` 모듈 docstring이 명문화된 단일 출처다.
  기본 보존은 성공 run 최신 10개·30일, 실패 run 30일, 로그 14일이며, `latest`가 가리키는
  run은 보호된다. 정리 루틴은 파이프라인이 자동 호출하지 않고 **운영자/스케줄러가 명시
  호출**해야 한다(하드 용량 캡은 없음 — 개수·나이로 간접 제한).
...
### P3 — `price_features.build_features`의 레거시 스캐폴딩 정리 ✅ 완료
- `legacy_removed_default_map`(약 40개 컬럼)을 0으로 추가했다가 마지막에 다시 drop하던
  방어 코드와, 그 기본값만 소비하다 곧바로 버려지던 파생 피처
  (`vi_after_return`·`vi_after_volume_spike`·`short_sell_event_score`·
  `shareholder_return_score`) 및 미사용 헬퍼(`_coerce_flag_series`·`_coerce_category_series`·
  `_warning_level_series`·`WARNING_LEVEL_MAP`)를 제거했다. 실제 입력 기준 출력 컬럼·선택
  피처·값이 리팩터 전후 **동일함을 지문 비교로 확인**했고, 원시 입력 정제용
  `drop_source_cols`는 계약 유지를 위해 보존했다.

### P3 — `result/` 산출물 관리 정책 명문화 ✅ 완료
- 보존 기간·정리 주기·용량 한계를 `src/utils/result_cleanup.py` 모듈 docstring에 단일
  출처로 명문화하고 §8에서 이를 가리키도록 했다. 정리 루틴이 자동 호출되지 않는다는
  점(수동/스케줄러 호출)과 하드 용량 캡 부재(개수·나이로 간접 제한)를 명시했다.
...
6. **레거시 정리(P3)** ✅ — 피처 스캐폴딩 제거(`price_features`)와 `result/` 보존 정책
   명문화(`result_cleanup` docstring + §8) 완료.
