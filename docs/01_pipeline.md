...
| `stock-predict-publish` | `src/ops/publish_predictions.py:main` | 기본 200종목 예측을 `published/`에 게시(GitHub 베이스라인) |
...
## 기준데이터 게시 (`stock-predict-publish`)

`src/ops/publish_predictions.py`가 파이프라인을 실행한 뒤 운영 산출물을 GitHub `published/`에 게시한다. 로컬에서 **수동 1회** 실행하는 흐름이며 cron은 없다.
...
| 단계 | 내용 |
|------|------|
| 1 | 실데이터 증분 갱신(`--full-refresh` 시 전체 재수집) 후 `run_pipeline` 실행 |
| 2 | `ensure_operational_manifest`로 운영 산출물(promoted + pass/warning) 검증 |
| 3 | 최신 run을 `published/latest/`와 `published/history/<거래일>/`로 복사 + `publish_meta.json` 기록 |
| 4 | `published/index.json`(가용 날짜·메타) 갱신 |
| 5 | `--no-push`/`--dry-run`이 아니면 `git add/commit/push` |

- `--news-mode`: `gemma`(기본, `localhost:8001` LLM + 규칙기반 자동 폴백) 또는 `rule`(규칙기반).
- 뉴스/공시 점수는 표시용이며 `predicted_return`·추천·신호 정책에 영향을 주지 않는다.
- 산출물·운영 절차 상세는 [07_reports.md](07_reports.md)와 `docs/OPERATIONS.md` 참고.
...
> 코드 분석으로 도출한 제안. 우선순위: **P0(정확성/버그) > P1(견고성/재현성) > P2(성능/품질/문서)**. 기준일: 2026-06-17.

### 미해결 — P1 `cfg.signal` 전역 in-place 변형 (재현성/직렬화 오염)

- **문제**: 튜닝 단계에서 `cfg.signal.return_weight = tuned[...]` 식으로 **공유 `AppConfig`를 직접 변형**한다(`src/pipeline.py:595-598`). 이후 `app_config_to_dict(cfg)`로 리포트에 직렬화되는 값이 "사용자가 준 설정"이 아니라 "튜닝 후 값"이 되어, 리포트만 보고 재현이 어렵다. 동일 프로세스에서 파이프라인을 2회 호출하면 1회차 튜닝값이 2회차 기본값으로 새어든다.
...
### 미해결 — P1 적응형 재시도 조건이 "폴드 0개"에만 동작

- **문제**: `_adaptive_training_cfg`는 walk-forward가 **폴드를 전혀 못 만들 때만** 재시도한다(`src/pipeline.py:553`, `_adaptive_training_cfg`는 `:145`). 폴드가 1~2개로 과소 생성되어도 그대로 진행되어 통계적으로 빈약한 검증 결과가 나온다.
- **제안**: `len(folds) < min_required_folds`(예: 3) 조건에서도 적응형 설정으로 재시도하고, 최종 폴드 수를 `diagnostics`에 노출.

### 미해결 — P2 `PipelineDiagnostics` 타이밍 커버리지 검증
...
> 참고: 이전 제안의 "`--add-symbols` KOSDAQ 종목 `.KS` 강제 변환" 항목은 `data/fetch_real_data.py`의 KRX 시장 매핑·`.KS`/`.KQ` 교차 폴백 적용으로 **해결됨**(`02_data.md` 이력 참고).
