...
# 선택: Colab에서 기본 다운로드 경로가 차단될 때 수동 ngrok 바이너리 준비
# !wget -O /tmp/ngrok.zip https://bin.equinox.io/c/bNyj1mQVY4c/ngrok-v3-stable-linux-amd64.zip
# !python - <<'PY'
# import zipfile, os
# with zipfile.ZipFile('/tmp/ngrok.zip') as zf:
#     zf.extract('ngrok', '/tmp')
# os.chmod('/tmp/ngrok', 0o755)
# PY
...
tunnel = launch_colab_kakao_bot(
    ...
    tunnel_config=PyngrokTunnelConfig(
        ...
        # ngrok_path="/tmp/ngrok",  # 수동 설치 시 지정(선택)
    ),
)
...
# 선택: Colab에서 ngrok 기본 다운로드 URL이 403/500이면 수동 설치
# !wget -O /tmp/ngrok.zip https://bin.equinox.io/c/bNyj1mQVY4c/ngrok-v3-stable-linux-amd64.zip
# !python - <<'PY'
# import zipfile, os
# with zipfile.ZipFile('/tmp/ngrok.zip') as zf:
