...
def test_recommendation_ignores_signal_probability_and_uncertainty_when_available():
    assert recommendation_from_signal(0.7, 0.8, up_probability=0.7, uncertainty_score=0.2) == "관망"
    assert recommendation_from_signal(0.2, 2.0, up_probability=0.6, uncertainty_score=0.2) == "관망"
    assert recommendation_from_signal(0.5, 1.2, up_probability=0.52, uncertainty_score=0.7) == "관망"
    assert recommendation_from_signal(1.0, -3.0, up_probability=1.0, uncertainty_score=0.0) == "매도"
    assert recommendation_from_signal(0.0, 3.0, up_probability=0.0, uncertainty_score=1.0) == "매수"
