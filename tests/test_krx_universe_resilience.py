...
from src.data.krx_universe import (
    _resolve_market_cap_col,
    _resolve_value_col,
    _top_by_market_cap,
    _top_by_trading_value,
    get_kospi200_kosdaq150_symbols,
)
...
def test_resolve_value_col_with_english_name():
    df = pd.DataFrame({"TradingValue": [3, 2, 1]}, index=["A", "B", "C"])
    assert _resolve_value_col(df) == "TradingValue"


def test_top_by_trading_value_handles_non_korean_schema():
    df = pd.DataFrame({"value": [10, 30, 20]}, index=["000001", "000002", "000003"])
    out = _top_by_trading_value(df, 2)
    assert out == ["000002", "000003"]


def test_get_kospi200_kosdaq150_symbols_prefers_index_constituents(monkeypatch):
    class _Stock:
        def get_market_ohlcv_by_ticker(self, date, market=None):
...
def test_get_kospi200_kosdaq150_symbols_with_trading_value_fallback(monkeypatch):
    class _Stock:
        def get_market_ohlcv_by_ticker(self, date, market=None):
            count = 220 if market == "KOSPI" else 180
            idx = [f"{i:06d}" for i in range(1, count + 1)]
            vals = list(range(count, 0, -1))
            return pd.DataFrame({"TradingValue": vals}, index=idx)

        def get_index_portfolio_deposit_file(self, index_code, date=None):
            raise RuntimeError("index constituent api unavailable")

        def get_market_cap_by_ticker(self, date, market="KOSPI"):
            raise KeyError("legacy korean column names are not available")
