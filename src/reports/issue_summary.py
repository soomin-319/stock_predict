...
목표:
- 공시 원문에 있는 핵심 사실(수치, 계약, 일정, 변경사항)을 빠짐없이 정리한다.
- 해석, 전망, 투자 판단은 하지 않는다.

핵심 규칙:
- 입력에 없는 정보는 절대 추가하지 마라
- 수치(금액, 비율, 규모)는 정확히 유지하라
- 공시 제목보다 본문 내용을 기준으로 정리하라
- 정정공시는 변경된 내용 위주로 정리하라
- 각 bullet은 한 문장으로 작성하라
- 중복 내용은 제거하라

공시 정리 기준:
- 아래 항목이 있으면 우선 반영하라:
  계약 규모, 상대방, 기간, 발행 조건, 자금조달 목적,
  지분 변화, 주요 일정, 변경 전후 내용, 재무 수치
...
NEWS_SUMMARY_PROMPT = """너는 종목 관련 뉴스를 투자 전문가에게 전달하기 위해 정리하는 AI다.

목표:
- 여러 기사에서 공통으로 확인되는 핵심 사실만 정리한다.
- 불필요한 서술 없이 사실만 간결하게 작성한다.
...
🚫 반드시 금지:
- "보도", "기사", "언급", "전해졌다", "나타났다" 같은 출처 표현
- "한 기사에서", "복수 기사에서" 같은 메타 표현
- 해석, 전망, 평가, 추정 표현

뉴스 정리 기준:
- 사실만 남기고 불필요한 설명 제거
- 기사라는 개념을 제거하고 사건 자체만 서술
- 자극적인 표현은 중립적으로 수정
...
추가 지시:
- "보도", "기사", "언급"이라는 단어가 포함되면 잘못된 출력이다.
- 해당 단어를 사용하지 말고 사실만 다시 작성하라.

이제 뉴스를 요약하라."""


COMBINED_SUMMARY_PROMPT = """You summarize Korean listed-company disclosures and news for investment research.
Do not add facts absent from the input. Preserve numbers exactly. Do not provide forecasts, investment advice, or buy/sell judgment.
Return JSON only with two keys: disclosure_summary and news_summary.
Each value must start with '[disclosure summary]' or '[news summary]' and contain up to five '- ' bullets of key facts.
If no key fact exists, return the header plus one bullet: '- no confirmed key disclosure content' or '- no confirmed key news content'."""
...
def _llm_symbol_issue_summary(
    *,
    ...
    try:
        ...
        print(f"[ISSUE SUMMARY] LLM summary failed ({symbol}): {type(exc).__name__}: {exc}")
        return None
